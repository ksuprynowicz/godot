 ...
 * wallpaper. Modified by Godot Engine.
 *
...
void wp_print_window(char const *prefix, wnd_t wnd, FILE *f) {
	...
	*name = 0;
	...
	print_verbose(vformat("%s[%08d] %s", prefix, wndid, classname));
	print_verbose(vformat("%4d, %4d, %4d, %4d | %s", (int32_t)rect.left, (int32_t)rect.top, (int32_t)rect.right, (int32_t)rect.bottom, name));
}
...
BOOL CALLBACK find_worker(wnd_t wnd, LPARAM lp) {
	...
	*pworker = FindWindowExA(0, wnd, "WorkerW", 0);
	if (*pworker) {
		print_verbose(vformat("The wallpaper is %08d", *pworker));
		print_verbose(vformat("Its parent is %08d", wnd));
		return FALSE;
	}
...
wnd_t wp_id() {
	...
	print_verbose("Scanning for program manager");
	progman = FindWindowA("Progman", 0);

	if (!progman) {
		print_error(vformat("Failed to find program manager, GLE=%08d", (uint32_t)GetLastError()));
		return 0;
	}

	print_verbose("Spawning wallpaper");
	...
	print_verbose("Checking for wallpaper");
	EnumWindows(find_worker, (LPARAM)&worker);

	if (!worker) {
		print_error("W: Couldn't spawn WorkerW window, trying old method");
	...
	if (worker && !is_win8_or_later()) {
		print_verbose("Detected Windows 7, hiding the worker window");
		ShowWindow(worker, SW_HIDE);
		worker = progman;
	}

	if (!worker) {
		print_error("W: Could not spawn window behind icons, falling back to the program manager");
		worker = progman;
	}
...
wnd_t wp_pick_window(int *keys, int *cancel_keys, int poll_ms) {
	...
	if (!GetCursorPos(&pos)) {
		print_error(vformat("GetCursorPos failed, GLE=%08d", (uint32_t)GetLastError()));
		return 0;
	}
...
int update_window_styles(wnd_t wnd, int32_t and, int32_t ex_and, int32_t or, int32_t ex_or) {
	unsigned gle;
	int32_t style = 0;
	int32_t exstyle = 0;
	...
	if (!style) {
		...
		if ((!style || !exstyle) && gle) {
			print_error(
					vformat("GetWindowLongA failed, GLE=%08d",
		...
		if (!SetWindowLongA(wnd, GWL_STYLE, style) ||
			...
			gle = GetLastError();
			if (gle) {
				print_error(vformat("SetWindowLongA failed, GLE=%08d", (uint32_t)gle));
				return 1;
			}
		}
...
int wp_add(wnd_t wnd) {
	...
	*wndclass = 0;
	...
	if (wallpaper == wnd || !strcmp(wndclass, "Shell_TrayWnd")) {
		print_error("Can't add this window");
		return 1;
	}

	if (IsChild(wallpaper, wnd)) {
		print_error("Already added");
		return 1;
	}

	print_verbose(vformat("Adding %08d", wnd));
	...
	if (!SetParent(wnd, wallpaper)) {
		print_error(vformat("SetParent failed, GLE=%08d", (uint32_t)GetLastError()));
		return 1;
	}
...
int wp_del(wnd_t wnd) {
	...
	if (wallpaper == wnd) {
		print_error("Can't delete this window.");
		return 1;
	}

	if (!IsChild(wallpaper, wnd)) {
		print_error("Already removed.");
		return 1;
	}

	print_verbose(vformat("Removing %08d", wnd));
	...
	or = (int32_t)WS_OVERLAPPEDWINDOW;
	ex_or = (int32_t)WS_EX_APPWINDOW;
...
int wp_kill(wnd_t wnd) {
	...
	if (!pid) {
		print_error("No process found.");
		return 1;
	}

	process = OpenProcess(PROCESS_TERMINATE, 0, pid);
	if (!process) {
		print_error(vformat("OpenProcess failed, GLE=%08d", (uint32_t)GetLastError()));
		return 1;
	}

	if (!TerminateProcess(process, 0)) {
		print_error(vformat("TerminateProcess failed, GLE=%08d", (uint32_t)GetLastError()));
		return 1;
	}
...
BOOL CALLBACK enum_window(wnd_t wnd, LPARAM lp) {
	...
	if (s->n >= s->nmax) {
		print_error(vformat("The enum_window exceeded the number max of %d", s->nmax));
		return FALSE;
	}
...
WEEBAPI int wp_list(wnd_t parent, wnd_t *result, int nmax) {
	...
	gle = GetLastError();
	if (!succ && gle) {
		print_error(vformat("EnumChildWindows failed, GLE=%08d", (uint32_t)gle));
		return -1;
	}
...
int wp_get_rect(wnd_t wnd, rect_t *rect) {
	if (!GetWindowRect(wnd, (RECT *)rect)) {
		print_error(vformat("GetWindowRect failed, GLE=%08d", (uint32_t)GetLastError()));
		return 1;
	}
	return 0;
}
...
int wp_move(wnd_t wnd, long left, long top, long right, long bottom) {
	...
	if (!succ) {
		print_error(vformat("SetWindowPos failed, GLE=%08d", (uint32_t)GetLastError()));
		return 1;
	}
	return 0;
}
...
int wp_fullscreen(wnd_t wnd) {
	...
	if (!mon) {
		print_error(vformat("MonitorFromWindow failed, GLE=%08d", (uint32_t)GetLastError()));
		return 1;
	}
	...
	if (!GetMonitorInfoA(mon, &mi)) {
		print_error(vformat("GetMonitorInfoA failed, GLE=%08d", (uint32_t)GetLastError()));
		return 1;
	}
...
int wp_exec(char const *file, char const *params) {
	if (!ShellExecuteA(0, "open", file, params, 0, 0)) {
		print_error(vformat("ShellExecuteA filed, GLE=%08d", (uint32_t)GetLastError()));
		return 1;
	}
...
handle_t wp_mutex(char const *name) {
	...
	if (!res) {
		print_error(vformat("CreateMutexA failed, GLE=%08d", (uint32_t)GetLastError()));
	}
